...
(function ($) {
    ...
    $.widget('ctg.imageViewer', {
        ...
        options: {
            ...
            zoomSteps: 3,
            imageUrlFormula: function () {
                throw "jQuery.ctg.imageViewer: You are required to supply a function for 'imageUrlFormula'"
            },
        ...
        _events: function () {
            ...
            this.objects.previews.find(this.opts.previewLinkSelector).each(function (i) {
        ...
        _initializeImages: function () {
            ...
            this.images = $.map(this.options.images, function (src) {
                return {
                    detail: self.options.imageUrlFormula(src, dims.detail[0], dims.detail[1]),
                    zoom: self.options.imageUrlFormula(src, dims.zoom[0], dims.zoom[1])
                };
            });
        ...
        _zoomToLevel: function (e) {
            ...
            this.objects.zoomImage.add(this.objects.decoyZoomImage)
                .animate({
                    left:     offset.x,
                    top:      offset.y,
                    width:    dims.w,
                    ...
                    complete:  function () {
                        ...
                        if (self.zoomLevel === 0) {
                            if (self.objects.zoomImage) {
                                self.objects.zoomImage.remove();
                                self.objects.zoomImage = null;
                            }

                            if (self.objects.decoyZoomImage) {
                                self.objects.decoyZoomImage.remove();
                                self.objects.decoyZoomImage = null;
                            }
                        }
        ...
        _loadZoomImage: function () {
            ...
            this.objects.decoyZoomImage = this.objects.detailImage.clone().insertBefore(this.objects.blocker);
            this.objects.zoomImage      = this.objects.detailImage.clone().insertBefore(this.objects.blocker);

            $(new Image())
                .bind('load', function () {

                    self.objects.zoomImage
                        .attr('src', this.src)
                        .css({
                            opacity: 0.00
                        })
        ...
        _resetImageAttributes: function () {
            ...
            this.objects.decoyZoomImage = null;
